...
# Monkey patching
import bcc.table
bcc.table._print_log2_hist = lambda vals, vt, _: print_hist(vals, vt)
...
def print_hist(vals, val_type):
    ...
    name = hist_filename.strip()
    filename = name.replace('<', '_').replace('>', '_')
    with open(filename + '.svg', 'w+') as fout:
        barchart.render(fout, name, val_type, val_max, rows)
...
while (1):
    ...
    if need_key:
        dist.print_log2_hist(label, "", section_print_fn=print_section,
